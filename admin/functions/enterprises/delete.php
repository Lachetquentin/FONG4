...
$cart_check_query = "SELECT * FROM cart_services WHERE id_user='$id' LIMIT 1";
$result = mysqli_query($db, $user_check_query);
$result2 = mysqli_query($db, $cart_check_query);

if (mysqli_num_rows($result2) > 0) {
    header('location: ../../enterprises.php?error=1');
} else if (mysqli_num_rows($result) > 0) { 
    $query = "DELETE FROM user WHERE id_user='$id'";
    mysqli_query($db, $query);
    header('location: ../../enterprises.php?success=2');
}
// register users if there are no errors in the form
else {
    // Alert non existant
    header("Location: ../../enterprises.php?error=2");
}
