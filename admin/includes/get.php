...
function getServiceById($service_id) {
    global $db;
    $sql = "SELECT services.* FROM services WHERE services.id_service='$service_id' LIMIT 1 ";
    $result = mysqli_query($db, $sql);
...
function getRoles() {
    global $db;
    $sql = "SELECT * FROM role WHERE id_role = 1 OR id_role = 2";
    $result = mysqli_query($db, $sql);
...
function addToCartProduct($userId, $productId, $stripeId, $quantity) {

    $check = checkIfExistProduct($userId, $productId);
    if ($check == true){
        /* Connexion à la bdd */
        $bdd = new PDO("mysql:host=localhost;dbname=forceofnature", "root", "");
        /* Préparation de la requête */
        $req = $bdd -> prepare('INSERT INTO cart_products(id_product, id_user, quantity, id_stripe) VALUES(?,?,?,?)');
        /* Définir les paramètres 1er p intérogation = le 1er id_user */
        $req->bindParam(1, $productId, PDO::PARAM_STR, 255);
        $req->bindParam(2, $userId, PDO::PARAM_STR, 255);
        $req->bindParam(3, $quantity, PDO::PARAM_STR, 255);
        $req->bindParam(4, $stripeId, PDO::PARAM_STR, 255);
        /* Executer la requête */ 
        $req->execute(); 
    } else {
        $quant = getProductQuantity($userId, $productId);
        $quant = (int)$quant+$quantity;
        $bdd = new PDO("mysql:host=localhost;dbname=forceofnature", "root", "");
        $req = $bdd -> prepare('UPDATE cart_products SET quantity= ? WHERE id_user=? AND id_product=?');
        $req->bindParam(1, $quant, PDO::PARAM_STR, 255);
        $req->bindParam(2, $userId, PDO::PARAM_STR, 255);
        $req->bindParam(3, $productId, PDO::PARAM_STR, 255);
        $req->execute();
    }
    
}

function checkIfExistProduct($userId, $productId){
    global $db;
    $sql = "SELECT COUNT(*) FROM cart_products WHERE id_product = $productId AND id_user = $userId";
    $query = mysqli_query($db, $sql);
    $res = mysqli_fetch_array($query);
    $count = $res['COUNT(*)'];
    if($count > 0){
...
function getProductQuantity($userId, $productId){
    /*global $db;
    $sql = "SELECT quantity FROM cart_products WHERE id_product = $productId AND id_user = $userId ";
    $result = mysqli_query($db, $sql);
    ...
    $username = "root";
    $password = "";
    $database = "forceofnature";
    $conn = new mysqli($servername, $username, $password, $database);
    // Check connection
    if ($conn->connect_error) {
        die("Connection failed: " . $conn->connect_error);
    }
    $query = "select quantity from cart_products where id_product = $productId AND id_user = $userId";
...
function addToCartService($userId, $serviceId, $stripeId, $quantity) {

    $check = checkIfExistService($userId, $serviceId);
    if ($check == true){
        /* Connexion à la bdd */
        $bdd = new PDO("mysql:host=localhost;dbname=forceofnature", "root", "");
        /* Préparation de la requête */
        $req = $bdd -> prepare('INSERT INTO cart_services(id_service, id_user, quantity, id_stripe) VALUES(?,?,?,?)');
        /* Définir les paramètres 1er p intérogation = le 1er id_user */
        $req->bindParam(1, $serviceId, PDO::PARAM_STR, 255);
        $req->bindParam(2, $userId, PDO::PARAM_STR, 255);
        $req->bindParam(3, $quantity, PDO::PARAM_STR, 255);
        $req->bindParam(4, $stripeId, PDO::PARAM_STR, 255);
        /* Executer la requête */ 
        ...
        $quant = getServiceQuantity($userId, $serviceId);
        $quant = (int)$quant+ $quantity;
        $bdd = new PDO("mysql:host=localhost;dbname=forceofnature", "root", "");
        $req = $bdd -> prepare('UPDATE cart_services SET quantity= ? WHERE id_user=? AND id_service=?');
        $req->bindParam(1, $quant, PDO::PARAM_STR, 255);
        $req->bindParam(2, $userId, PDO::PARAM_STR, 255);
        $req->bindParam(3, $serviceId, PDO::PARAM_STR, 255);
        $req->execute();
    }
    
}

function checkIfExistService($userId, $serviceId){
    global $db;
    $sql = "SELECT COUNT(*) FROM cart_services WHERE id_service = $serviceId AND id_user = $userId";
    $query = mysqli_query($db, $sql);
    $res = mysqli_fetch_array($query);
    $count = $res['COUNT(*)'];
    if($count > 0){
...
function getServiceQuantity($userId, $serviceId){
    /*global $db;
    $sql = "SELECT quantity FROM cart_products WHERE id_product = $productId AND id_user = $userId ";
    $result = mysqli_query($db, $sql);
    $quantity = mysqli_fetch_all($result, MYSQLI_ASSOC);
    ...
    $query = "select quantity from cart_services where id_service = $serviceId AND id_user = $userId";
    $queryResult = $conn->query($query);
    while ($queryRow = $queryResult->fetch_row()) {
        for($i = 0; $i < $queryResult->field_count; $i++){
            return $queryRow[$i];
...
function stripeLaunch($userId){
    ...
    $query = "select id_stripe from cart_products Where id_user=$userId;";
    $queryResult = $conn->query($query);
    while ($queryRow = $queryResult->fetch_row()) {
        for($i = 0; $i < $queryResult->field_count; $i++){
            $query1 = "select quantity from cart_products Where id_user=$userId AND id_stripe='$queryRow[$i]';";
            $queryResult1 = $conn->query($query1);
            while ($queryRow1 = $queryResult1->fetch_row()) {
                for($i = 0; $i < $queryResult1->field_count; $i++){
                    echo"{
                        price:'$queryRow[$i]',
                        quantity: $queryRow1[$i]
                    },";
                }
            }
        }
...
function stripeLaunchServices($userId){
    $servername = "localhost";
    $username = "root";
    $password = "";
    $database = "forceofnature";
    ...
    $query = "select id_stripe from cart_services Where id_user=$userId;";
    $queryResult = $conn->query($query);
    while ($queryRow = $queryResult->fetch_row()) {
        for($i = 0; $i < $queryResult->field_count; $i++){
            $query1 = "select quantity from cart_services Where id_user=$userId AND id_stripe='$queryRow[$i]';";
            $queryResult1 = $conn->query($query1);
            while ($queryRow1 = $queryResult1->fetch_row()) {
                for($i = 0; $i < $queryResult1->field_count; $i++){
                    echo"{
