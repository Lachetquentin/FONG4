...
if (isset($_POST['addBtn'])) {
    ...
    $enterprise = isset($_POST['enterprise']) ? htmlspecialchars($_POST['enterprise']) : "";
    ...
    if ($password == $confirmpw) {
        ...
        if ($firstname && $lastname && $cryptedpw && $phonenumber && $email && $address && $zipcode && $idtype && $idcountry && $enterprise) {
            $result = mysqli_num_rows(mysqli_query($db, "SELECT email from user WHERE email='$email'"));
            if ($result > 0) {
                header('Location: inscriptionEntreprises.php?error=1');
                
            }

            mysqli_query($db, "INSERT INTO user (id_role, id_country, id_type, first_name, last_name, address, zip_code, email, tel, password, enterprise) VALUES ('4', '$idcountry', '$idtype', '$firstname', '$lastname', '$address', '$zipcode', '$email', '$phonenumber' , '$cryptedpw', '$enterprise')");
            header("Location: connexionEntreprises.php?success=1");
            
        } else {
            header("Location: inscriptionEntreprises.php?error=2");
            
        }
    } else {
        header("Location: inscriptionEntreprises.php?error=3");
...
<body class="d-flex flex-column min-vh-100">
    ...
    <main>
        ...
        ?>
        <form action="inscriptionEntreprises.php" method="POST" class="d-flex justify-content-center">
            <div class="form mw-100 m-5 p-4 b-radius shadow">
                ...
                <div class="form-group d-flex justify-content-center align-items-center mt-2">
                    <i class="material-icons">phone</i>
                    <input type="text" name="enterprise" class="form-control shadow-sm m-2" aria-describedby="enterprise" placeholder="Entreprise*" value="<?php echo isset($enterprise) ? $enterprise : ""; ?>" required>
                </div>
                <div class="form-group d-flex justify-content-center align-items-center mt-2">
